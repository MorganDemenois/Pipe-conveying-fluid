...
#             eigenVectors = np.concatenate((eigenVectors[:,:l], eigenVectors[:,l+1:]), axis = 1)
#         else:    
#             l += 1


IM_Omega = np.array([(-1j*eigenValues).imag])
RE_Omega = np.array([(-1j*eigenValues).real])
Vectors = np.array([eigenVectors])
...
while u < umax:
    ...
    while min(np.max(diff,1)) < 0.8 and du_tempo > 10**-10:
    ...
    # for k in range(N):
    #     if MAC(Vectors[i-1,k],Vectors[i,k]) < 0.9:
    #         print(IM_Omega[i,k]-IM_Omega[i-1,k])
    #         print(RE_Omega[i,k]-RE_Omega[i-1,k])
    #         print(MAC(Vectors[i-1,k],Vectors[i,k]))
